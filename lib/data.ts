...
function buildSplits(index: number): StationSplit[] {
  ...
  return stationDefinitions.map((station, stationIndex) => {
    ...
    return {
      key: station.key,
      label: station.label,
      seconds: Math.round(station.baseSeconds * fitnessFactor * runDiscipline * stationModifier * lateRace * noise),
    };
  });
}
...
export const athletes: Athlete[] = Array.from({ length: 50 }, (_, index) => {
  ...
  return {
    ...
    division,
    weightDivision: division === "Pro" || index % 4 === 0 ? "Pro Weights" : "Open Weights",
